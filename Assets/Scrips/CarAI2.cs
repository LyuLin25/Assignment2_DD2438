...
namespace UnityStandardAssets.Vehicles.Car
{
    ...
    public class CarAI2 : MonoBehaviour
    {
		...
		{
			if ((once || coord.firstPath.Count != path.Count) && this.gameObject.name == "ArmedCar (1)") {
				path = coord.firstPath;
				if (path != null) {
					once = false;
				}
			}else if ((once || coord.secondPath.Count != path.Count) && this.gameObject.name == "ArmedCar (2)") {
				path = coord.secondPath;
				if (path != null) {
					once = false;
				}
			}else if ((once || coord.thirdPath.Count != path.Count) && this.gameObject.name == "ArmedCar (3)") {
